...
function sel_data_single_work_order_controller()
{
    global $conn;
    foreach ($_REQUEST as $key => $val) {
        global $$key;
    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if ((count($result_h) > 0) && (count($result_m) > 0)) {
            for ($n = 0; $n < count($result_m); $n++) {
                ...
                $toatl_machine_cost += $real_data[$i]["mcm_work_time"] * $result_m[$n]['mc_min_rate'];
            }
        }
    }
    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if ((count($result_h) > 0) && (count($result_m) > 0)) {
            for ($n = 0; $n < count($result_m); $n++) {
                ...
                $machine_cost = $real_data[$i]["mcm_work_time"] * $result_m[$n]['mc_min_rate'];
                $add_cost = $human_cost + $machine_cost;
                $tmp = array(
                    //日期
                    "REAL_DATE" => $real_data[$i]['YEAR'] . '-' . $real_data[$i]['MONTH'],
                    //工單號
                    "ORDER_NO" => $real_data[$i]["ORDER_NO"],
                    //成品料號
                    "PART_NO" => $real_data[$i]['PART_NO'],
                    //直接人工編號
